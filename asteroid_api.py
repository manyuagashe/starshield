    ...
    try:
        ...
        for i, record in enumerate(train_data):
            try:
                ...
                # Create AsteroidFeatures with proper error handling
                asteroid_data = {
                    'object_id': record.get('object_name', f'TRAIN-{i}'),
                    'name': record.get('object_name', f'TRAIN-{i}'),
                    'distance_au': float(record.get('distance_au', 0.1)),
                    'velocity_kms': float(record.get('velocity_kms', 10.0)),
                    'diameter_km': float(record.get('diameter_km', 0.1)),
                    'v_infinity_kms': float(record.get('v_infinity_kms', 10.0)),
                    'is_pha': bool(record.get('is_pha', False)),
                    'orbit_class': record.get('orbit_class', 'APO'),
                    'eta_closest': record.get('approach_date'),
                    'impact_probability': None
                }
                
                asteroids.append(AsteroidFeatures(**asteroid_data))
        ...
        if not asteroids:
            raise HTTPException(
                status_code=500,
                detail="No valid asteroid records could be processed"
            )
    except:
...
def get_all_predictions(
    ...
    try:
        ...
        logger.info("Fetching all data from NASA API...")
        train_data, test_data = get_train_test_data()
        logger.info(f"Successfully fetched {len(train_data)} training and {len(test_data)} test records from API")
